...
app.post('/places', async (req, res) => {
  try {
    ...
    const body = {
      ...
      regionCode: 'IT',
      maxResultCount: 20,
      fields: 'places.displayName,places.formattedAddress,places.nationalPhoneNumber,places.websiteUri,places.rating,places.userRatingCount,places.businessStatus,places.editorialSummary,places.types,places.photos'
    };
...
app.post('/seo-analyze', async (req, res) => {
  try {
    ...
    const hasSito = !!web;
    const prompt = `Sei un esperto di digital marketing italiano. Analizza la presenza digitale di questa attivitÃ  e rispondi SOLO con un oggetto JSON valido, senza testo aggiuntivo prima o dopo.
...
Tipo attivitÃ : ${(tipi || []).slice(0,3).join(', ') || 'attivitÃ  locale'}

${hasSito ? 'Analizza la presenza digitale considerando le best practice SEO 2024 per il mercato italiano.' : 'Il sito web Ã¨ assente â€” questo Ã¨ giÃ  un problema critico che impatta tutto il resto.'}
...
Rispondi SOLO con questo JSON (nessun testo fuori):
{
  ...
  "seo_items": [
    ...
    {"label": "VisibilitÃ  Google Maps", "stato": "<ok|warning|error>", "valore": "<stima visibilitÃ  locale>"},
    {"label": "Recensioni", "stato": "<ok|warning|error>", "valore": "<${nRating} recensioni â€” valutazione>"},
  ...
  "opportunita": ["<opportunitÃ  specifica 1>", "<opportunitÃ  specifica 2>", "<opportunitÃ  specifica 3>"],
  "servizi_urgenti": ["<servizio Pagine SÃ¬! consigliato 1>", "<servizio 2>"]
}`;
