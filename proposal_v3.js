...
function esportaPDF() {
  ...
  document.querySelectorAll('#tabella-prodotti tbody tr').forEach(function(tr) {
    ...
    var anno1 = parseFloat(tr.dataset.anno1 || 0);
    var mens = parseFloat(tr.dataset.mens || 0);
    rows.push({ sigla: sigla, nome: nome, anno1: anno1, mens: mens });
  });
  var totAnno1 = rows.reduce(function(s,r){ return s + r.anno1; }, 0);
  var totMens = rows.reduce(function(s,r){ return s + r.mens; }, 0);
  var oggi = new Date().toLocaleDateString('it-IT', { day: '2-digit', month: 'long', year: 'numeric' });

  var righeHTML = rows.map(function(r) {
    return '<tr>' +
      ...
      '<td style="padding:10px 14px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#555;text-align:right">' + (r.anno1 ? '&euro; ' + r.anno1.toLocaleString('it-IT') : '&mdash;') + '</td>' +
      '<td style="padding:10px 14px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#555;text-align:right">' + (r.mens ? '&euro; ' + r.mens + '/mese' : '&mdash;') + '</td>' +
      '</tr>';
  }).join('');

  var mensHTML = totMens ? '<div style="text-align:right"><div style="font-size:9px;opacity:0.7;margin-bottom:2px">Mensile</div><div style="font-size:18px;font-weight:700">&euro; ' + totMens + '/mese</div></div>' : '';

  var pdfHTML = '<!DOCTYPE html><html lang="it"><head><meta charset="UTF-8">' +
    '<title>Proposta &mdash; ' + nomeAz + '</title>' +
    ...
    '<div style="background:#111;color:white;padding:24px 28px;border-radius:10px;margin-bottom:28px;display:flex;align-items:center;justify-content:space-between">' +
      '<div>' +
        ...
        '<div style="font-size:11px;color:rgba(255,255,255,0.5)">Proposta commerciale &middot; ' + oggi + '</div>' +
      '</div>' +
      '<div style="font-size:11px;color:#E8001C;font-weight:600;letter-spacing:0.05em">PAGINE S&Igrave;!</div>' +
    '</div>' +
    ...
    '<div style="background:#E8001C;color:white;border-radius:8px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center">' +
      ...
      '<div style="display:flex;gap:32px;align-items:center">' +
        '<div style="text-align:right">' +
          ...
          '<div style="font-size:18px;font-weight:700">&euro; ' + totAnno1.toLocaleString('it-IT') + '</div>' +
        '</div>' +
        mensHTML +
      '</div>' +
    '</div>' +
    '<div style="margin-top:20px;font-size:9px;color:#aaa;text-align:center">Prezzi IVA esclusa &middot; Valida 30 giorni &middot; Pagine S&igrave;! SpA</div>' +
    '</body></html>';
