...
<div class="body">
  ...
  <div class="edit-note no-print">
    ...
    <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap">
      <button onclick="esportaPDF()" style="padding:6px 16px;background:#E8001C;color:#fff;border:none;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer">📄 Esporta PDF sintetico</button>
      <button onclick="window.print()" style="padding:6px 16px;background:#fff;color:#333;border:1.5px solid #ddd;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer">🖨 Stampa pagina completa</button>
    </div>
  </div>
  ...
  <div class="no-print" style="margin-bottom:20px">
    <button onclick="apriPannello()" style="display:inline-flex;align-items:center;gap:6px;padding:8px 18px;background:#fff;border:2px dashed #E8001C;color:#E8001C;border-radius:8px;font-size:11px;font-weight:600;cursor:pointer;transition:all 0.15s" onmouseover="this.style.background='#fff0f2'" onmouseout="this.style.background='#fff'">
...
function ricalcolaTotali() {
  ...
  document.getElementById('tot-mens').textContent = mens ? mens : '—';
  document.getElementById('tot-num').textContent = num;
}

function esportaPDF() {
  // Raccoglie dati dalla proposta corrente
  const nomeAz = document.querySelector('.az-nome') ? document.querySelector('.az-nome').textContent : 'Cliente';
  const rows = [];
  document.querySelectorAll('#tabella-prodotti tbody tr').forEach(tr => {
    const sigla = tr.querySelector('.td-sigla') ? tr.querySelector('.td-sigla').textContent.trim() : '';
    const nome = tr.querySelector('.p-nome') ? tr.querySelector('.p-nome').textContent.trim() : '';
    const anno1 = parseFloat(tr.dataset.anno1 || 0);
    const mens = parseFloat(tr.dataset.mens || 0);
    rows.push({ sigla, nome, anno1, mens });
  });
  const totAnno1 = rows.reduce((s, r) => s + r.anno1, 0);
  const totMens = rows.reduce((s, r) => s + r.mens, 0);
  const oggi = new Date().toLocaleDateString('it-IT', { day: '2-digit', month: 'long', year: 'numeric' });

  const righeHTML = rows.map(r => `
    <tr>
      <td style="padding:10px 14px;border-bottom:1px solid #f0f0f0;font-family:monospace;font-size:10px;color:#E8001C;font-weight:700">${r.sigla}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#1a1a1a;font-weight:500">${r.nome}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#555;text-align:right">${r.anno1 ? '€ ' + r.anno1.toLocaleString('it-IT') : '—'}</td>
      <td style="padding:10px 14px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#555;text-align:right">${r.mens ? '€ ' + r.mens + '/mese' : '—'}</td>
    </tr>`).join('');
...
<head>
<meta charset="UTF-8">
<title>Proposta — \${nomeAz}</title>
<style>
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Inter',sans-serif;color:#1a1a1a;padding:40px 48px;max-width:800px;margin:0 auto}
@media print{body{padding:20px 30px;print-color-adjust:exact;-webkit-print-color-adjust:exact}}
</style>
</head>
<body>
<div style="background:#111;color:white;padding:24px 28px;border-radius:10px;margin-bottom:28px;display:flex;align-items:center;justify-content:space-between">
  <div>
    <div style="font-size:18px;font-weight:700;margin-bottom:4px">\${nomeAz}</div>
    <div style="font-size:11px;color:rgba(255,255,255,0.5)">Proposta commerciale · \${oggi}</div>
  </div>
  <div style="font-size:11px;color:#E8001C;font-weight:600;letter-spacing:0.05em">PAGINE SÌ!</div>
</div>

<table style="width:100%;border-collapse:collapse;margin-bottom:24px">
  <thead>
    <tr style="background:#f5f5f5">
      <th style="padding:10px 14px;text-align:left;font-size:9px;font-weight:600;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Sigla</th>
      <th style="padding:10px 14px;text-align:left;font-size:9px;font-weight:600;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Soluzione proposta</th>
      <th style="padding:10px 14px;text-align:right;font-size:9px;font-weight:600;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Anno 1</th>
      <th style="padding:10px 14px;text-align:right;font-size:9px;font-weight:600;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Mensile</th>
    </tr>
  </thead>
  <tbody>\${righeHTML}</tbody>
</table>

<div style="background:#E8001C;color:white;border-radius:8px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center">
  <div style="font-size:10px;font-weight:600;letter-spacing:0.08em;text-transform:uppercase;opacity:0.8">Investimento totale</div>
  <div style="display:flex;gap:32px;align-items:center">
    <div style="text-align:right">
      <div style="font-size:9px;opacity:0.7;margin-bottom:2px">Anno 1</div>
      <div style="font-size:18px;font-weight:700">€ \${totAnno1.toLocaleString('it-IT')}</div>
    </div>
    \${totMens ? '<div style="text-align:right"><div style="font-size:9px;opacity:0.7;margin-bottom:2px">Mensile</div><div style="font-size:18px;font-weight:700">€ ' + totMens + '/mese</div></div>' : ''}
  </div>
</div>

<div style="margin-top:20px;font-size:9px;color:#aaa;text-align:center">Prezzi IVA esclusa · Valida 30 giorni · Pagine Sì! SpA</div>
</body></html>\`;
...
<!-- PANNELLO AGGIUNGI SERVIZIO -->
<div id="pannello-overlay" onclick="if(event.target===this)chiudiPannello()" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.4);backdrop-filter:blur(3px);z-index:9999;align-items:center;justify-content:center;">
