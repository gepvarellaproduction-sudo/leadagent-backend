...
const PRODOTTI = {
  'Si2A-PM':    { nome: 'Si!2Site Atom', cat: 'Sito Web', desc: 'Sito CMS base - HP + gallery + contatti', anno1: 945, mens: 59 },
  'Si2RE-PM':   { nome: 'Si!2Site Ready', cat: 'Sito Web', desc: 'Sito CMS completo con blog e gallery', anno1: 1545, mens: 93 },
  'Si2S-PM':    { nome: 'Si!2Site Super', cat: 'Sito Web', desc: 'Sito CMS avanzato con catalogo prodotti', anno1: 2150, mens: 127 },
  'Si2VN-PM':   { nome: 'Si!2Site Vertical', cat: 'Sito Web', desc: 'Sito WordPress professionale fino 10 pagine', anno1: 2350, mens: 141 },
  ...
  'WDSA':       { nome: 'Scheda Azienda', cat: 'Directory PagineSi.it', desc: 'Scheda completa - 45 kw + 2 schede attivita', anno1: 400, mens: 38 },
  ...
  'SIN':        { nome: 'SEO In Site', cat: 'SEO', desc: 'Posizionamento SEO su sito Pagine Si!', anno1: 1390, mens: 133 },
  ...
  'SIADVLS':    { nome: 'SiAdv Locale Setup', cat: 'Google Ads', desc: 'Setup campagna SiAdvertising Locale', anno1: 200, mens: null },
  'SIADVLG':    { nome: 'SiAdv Locale Gestione', cat: 'Google Ads', desc: 'Gestione mensile campagna SiAdvertising Locale', anno1: null, mens: 250 },
  ...
  'Si4BLD':     { nome: 'Si!4Business Lead', cat: 'Marketing Automation', desc: 'Piattaforma email/SMS marketing con 15.000 mail/mese', anno1: 790, mens: 76 },
  'Si4BEN':     { nome: 'Si!4Business Engage', cat: 'Marketing Automation', desc: 'Lead + SMS aggiuntivi + fidelity card + calendario', anno1: 990, mens: 95 },
};
...
const FATTURATO_PER_SETTORE = {
  ...
  default: { min: 100000, max: 400000, label: 'Attivita Locale' }
};
...
function stimaFatturato(lead) {
  ...
  return {
    settore, label: fascia.label,
    min: fatMin, max: fatMax, mid: fatMid,
    budgetMensileMax: Math.min(budgetMensile, 800),
    geo
  };
}
...
function analisiDigitale(lead) {
  ...
  return {
    hasSito, pocheRec, ratingBasso, ratingMedio,
...
function costruisciPreventivo(lead, fatturato, analisi) {
  ...
  if (budget < 200) aggiungi('WDSAL', 'Visibilita immediata su PagineSi.it', 1);
  else aggiungi('WDSA', 'Visibilita su PagineSi.it con 45 parole chiave', 1);
...
async function analisiSEOReale(lead) {
  ...
  if (!categoria || !citta) return null;
  ...
  try {
    const resp = await fetch('https://api.dataforseo.com/v3/serp/google/organic/live/regular', {
      ...
      body: JSON.stringify([{ keyword: keyword, location_code: 2380, language_code: 'it', depth: 100 }])
    });
    ...
    // Cerca posizione del lead
    const nomeNorm = (lead.nome || '').toLowerCase();
    const webNorm = lead.web && lead.web !== 'N/D'
      ? lead.web.toLowerCase().replace(/^https?:\/\/(www\.)?/, '').split('/')[0]
      : null;
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (webNorm && dom.includes(webNorm)) { posizioneReale = i + 1; urlTrovato = item.url; break; }
      if (nomeNorm.split(' ')[0] && tit.includes(nomeNorm.split(' ')[0]) && tit.includes(citta.toLowerCase())) {
        posizioneReale = i + 1; urlTrovato = item.url; break;
      }
    }

    // Competitor: prime 5 posizioni organiche che non siano directory o il lead stesso
    const directory = ['paginegialle', 'tripadvisor', 'paginebianche', 'virgilio', 'yelp', 'google', 'facebook', 'instagram'];
    const competitor = items
      .filter(item => {
        const dom = (item.domain || '').toLowerCase();
        const isDir = directory.some(d => dom.includes(d));
        const isLead = webNorm && dom.includes(webNorm);
        return !isDir && !isLead;
      })
      .slice(0, 3)
      .map((item, idx) => ({
        ...
        url: item.url,
        dominio: item.domain,
        punto_forza: idx === 0 ? 'Prima posizione organica' : idx === 1 ? 'Alta visibilita organica' : 'Buon posizionamento locale'
      }));

    return {
      keyword,
      posizione: posizioneReale,
      url_trovato: urlTrovato,
      trovato: !!posizioneReale,
      competitor,
      n_risultati: items.length
    };
  } catch(e) {
    return null;
  }
  ...
  try {
    ...
    return data.places
      .filter(p => {
        ...
        return nome && !nome.includes(nomeLeadNorm.slice(0, 6));
      })
      .slice(0, 3)
      .map((p, idx) => {
        ...
        return {
          ...
          ha_sito: hasSitoComp,
          punto_forza: ratingComp >= 4.3 ? 'Alta reputazione' : nRatingComp >= 100 ? 'Molte recensioni' : hasSitoComp ? 'Sito web attivo' : 'Solo Maps'
        };
      });
  } catch(e) {
    return [];
...
function buildAnalisiHTML(seo, mapsCompetitor, lead) {
  ...
  var socialLink = lead.socialLink || null;
  var hasSocial = !!socialLink;
  var isFb = socialLink && socialLink.includes('facebook');
  var isIg = socialLink && socialLink.includes('instagram');

  // Posizione Google
  var posizione, posizioneLabel, posLivello, posColore, posCommento, keyword;
  if (seo && seo.trovato) {
    posizione = seo.posizione;
    keyword = seo.keyword;
    posLivello = posizione <= 10 ? 'ALTO' : posizione <= 30 ? 'MEDIO' : 'BASSO';
    posColore = posizione <= 10 ? '#2e7d32' : posizione <= 30 ? '#f57f17' : '#b71c1c';
    posCommento = posizione <= 10 ? 'Prima pagina Google - ottima visibilita' : posizione <= 30 ? 'Pagine 2-3 Google - visibilita migliorabile' : 'Oltre la terza pagina - necessario intervento SEO';
    ...
    // Stima se DataForSEO non disponibile
    keyword = (lead.categoria || '') + ' ' + (lead.citta || '');
    if (!hasSito && nRating < 10) { posizione = '95+'; posLivello = 'BASSO'; posColore = '#b71c1c'; posCommento = 'Nessun sito e poche recensioni: praticamente invisibile'; }
    else if (!hasSito) { posizione = '60+'; posLivello = 'BASSO'; posColore = '#b71c1c'; posCommento = 'Senza sito web il posizionamento e molto limitato'; }
    else if (nRating >= 100) { posizione = '~10'; posLivello = 'MEDIO'; posColore = '#f57f17'; posCommento = 'Buona base ma ottimizzazione SEO necessaria'; }
    else { posizione = '40+'; posLivello = 'BASSO'; posColore = '#b71c1c'; posCommento = 'Posizionamento debole, margine di crescita alto'; }
  }
  ...
  function barra(label, before, after) {
    return '<div style="margin-bottom:12px">' +
      ...
      '<div style="display:flex;align-items:center;gap:8px">' +
        ...
        '<div style="flex:1;height:7px;background:#f0f0f0;border-radius:99px">' +
          '<div style="height:7px;width:' + after + '%;background:#4caf50;border-radius:99px"></div>' +
        '</div>' +
        '<span style="font-size:9pt;color:#4caf50;font-weight:700;width:40px;text-align:right">+' + after + '%</span>' +
      '</div>' +
    '</div>';
  }
  ...
  var boxStyle = 'background:#f9f9f9;border:1px solid #eee;border-radius:10px;padding:16px 20px;margin-bottom:20px';

  var h = '';

  // Header sezione
  h += '<div style="background:#f9f9f9;padding:14px 22px;border-bottom:1px solid #eee;margin:-22px -22px 20px -22px">';
  h += '<div style="font-size:10.5pt;font-weight:700;color:#E8001C;text-transform:uppercase;letter-spacing:0.06em">Analisi Digitale Prevendita</div>';
  h += '<div style="font-size:9pt;color:#aaa;margin-top:2px">Lead Agent - Dati reali Google Search e Maps</div>';
  h += '</div>';

  // Posizionamento Google
  h += '<div style="' + secStyle + '">Posizionamento Google</div>';
  h += '<div style="display:flex;align-items:stretch;gap:16px;flex-wrap:wrap;margin-bottom:20px">';
  h += '<div style="text-align:center;background:#f9f9f9;border:1px solid #eee;border-radius:10px;padding:20px 28px;flex-shrink:0">';
  h += '<div style="font-size:2.6rem;font-weight:800;color:' + posColore + ';line-height:1">' + posizione + '</div>';
  h += '<div style="font-size:8pt;color:#aaa;margin-top:5px;text-transform:uppercase;letter-spacing:0.06em">Posizione Google</div>';
  h += '</div>';
  h += '<div style="flex:1;min-width:180px;display:flex;flex-direction:column;justify-content:center">';
  h += '<div style="margin-bottom:8px"><span style="display:inline-block;padding:3px 12px;border-radius:12px;font-weight:700;font-size:10pt;background:' + (posLivello === 'ALTO' ? '#e8f5e9;color:#2e7d32' : posLivello === 'MEDIO' ? '#fff8e1;color:#f57f17' : '#fce8e8;color:#b71c1c') + '">' + posLivello + '</span></div>';
  h += '<div style="font-size:9.5pt;color:#555">Keyword: <strong>' + keyword + '</strong></div>';
  h += '<div style="font-size:9pt;color:#888;margin-top:4px">' + posCommento + '</div>';
  if (seo && seo.url_trovato) {
    h += '<div style="font-size:8.5pt;color:#aaa;margin-top:6px">URL: <a href="' + seo.url_trovato + '" style="color:#1565c0">' + seo.url_trovato + '</a></div>';
  }
  h += '</div></div>';

  // Social Media
  h += '<div style="' + secStyle + '">Presenza Social Media</div>';
  h += '<div style="' + boxStyle + '">';
  h += '<div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:8px">';
  var sCls = hasSocial ? 'background:#fff8e1;color:#f57f17' : 'background:#fce8e8;color:#b71c1c';
  h += '<span style="display:inline-block;padding:4px 14px;border-radius:20px;font-size:10.5pt;font-weight:700;' + sCls + '">' + (hasSocial ? 'PRESENTE' : 'ASSENTE') + '</span>';
  h += '<span style="font-size:9.5pt;color:#555">' + (hasSocial ? 'Profilo social trovato tramite Google Maps' : 'Nessun profilo social rilevato') + '</span>';
  h += '</div>';
  if (isFb) h += '<a href="' + socialLink + '" target="_blank" style="display:inline-block;margin-right:8px;padding:6px 16px;background:#1877f2;color:white;border-radius:20px;font-size:9.5pt;font-weight:600;text-decoration:none">Facebook</a>';
  if (isIg) h += '<a href="' + socialLink + '" target="_blank" style="display:inline-block;padding:6px 16px;background:#e1306c;color:white;border-radius:20px;font-size:9.5pt;font-weight:600;text-decoration:none">Instagram</a>';
  h += '</div>';

  // Competitor
  var competitor = [];
  if (seo && seo.competitor && seo.competitor.length) {
    // Competitor da SERP Google (con posizione reale)
    competitor = seo.competitor;
    h += '<div style="' + secStyle + '">Competitor - Dati Google Search Reali</div>';
    h += '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px">';
    competitor.forEach(function(c) {
      h += '<div style="flex:1;min-width:150px;border:1px solid #eee;border-radius:8px;padding:12px;background:#fafafa">';
      h += '<div style="font-size:9.5pt;font-weight:700;color:#1a1a1a;margin-bottom:8px">' + c.nome + '</div>';
      var pCls = c.posizione_google <= 10 ? 'background:#e8f5e9;color:#2e7d32' : c.posizione_google <= 30 ? 'background:#fff8e1;color:#f57f17' : 'background:#fce8e8;color:#b71c1c';
      h += '<div style="font-size:9pt;color:#777;margin-bottom:4px">Posiz. Google: <span style="padding:1px 7px;border-radius:10px;font-weight:700;font-size:8pt;' + pCls + '">#' + c.posizione_google + '</span></div>';
      h += '<div style="font-size:8.5pt;color:#aaa;margin-top:6px;font-style:italic">' + (c.punto_forza || '') + '</div>';
      if (c.url) h += '<div style="font-size:8pt;color:#1565c0;margin-top:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + c.dominio + '</div>';
      h += '</div>';
    });
    h += '</div>';
  } else if (mapsCompetitor && mapsCompetitor.length) {
    // Fallback: competitor da Google Maps
    h += '<div style="' + secStyle + '">Competitor - Dati Google Maps</div>';
    h += '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px">';
    mapsCompetitor.forEach(function(c, idx) {
      var pCls = idx === 0 ? 'background:#e8f5e9;color:#2e7d32' : idx === 1 ? 'background:#fff8e1;color:#f57f17' : 'background:#fce8e8;color:#b71c1c';
      h += '<div style="flex:1;min-width:150px;border:1px solid #eee;border-radius:8px;padding:12px;background:#fafafa">';
      h += '<div style="font-size:9.5pt;font-weight:700;color:#1a1a1a;margin-bottom:8px">' + c.nome + '</div>';
      h += '<div style="font-size:9pt;color:#777;margin-bottom:3px">Maps: <span style="padding:1px 7px;border-radius:10px;font-weight:700;font-size:8pt;' + pCls + '">#' + c.posizione_maps + '</span></div>';
      h += '<div style="font-size:9pt;color:#777;margin-bottom:3px">Rating: <strong>' + (c.rating || 'N/D') + '</strong> (' + (c.n_recensioni || 0) + ' rec.)</div>';
      h += '<div style="font-size:9pt;color:#777;margin-bottom:5px">Sito: <strong>' + (c.ha_sito ? 'Si' : 'No') + '</strong></div>';
      h += '<div style="font-size:8.5pt;color:#aaa;font-style:italic">' + (c.punto_forza || '') + '</div>';
      h += '</div>';
    });
    h += '</div>';
  }
  ...
  // Scenario crescita
  h += '<div style="' + secStyle + '">Scenario di Crescita Atteso</div>';
  h += '<div style="background:#111;border-radius:8px;padding:18px 20px;margin-bottom:20px">';
  h += '<div style="font-size:9.5pt;font-weight:600;color:rgba(255,255,255,0.7);margin-bottom:14px">Situazione Attuale vs Con i Servizi Proposti</div>';
  // Usiamo barre su sfondo bianco dentro il nero per evitare problemi di colore
  h += '<div style="background:rgba(255,255,255,0.05);border-radius:6px;padding:14px">';
  h += barra('Visibilita sito', sbSito, saSito);
  h += barra('Presenza social', sbSocial, saSocial);
  h += barra('Posiz. Google', sbGoogle, saGoogle);
  h += '</div></div>';

  // Opportunita
  var opps = [];
  if (!hasSito) opps.push('Creare sito web: primo passo per comparire su "' + keyword + '"');
  else if (posLivello !== 'ALTO') opps.push('SEO locale: scalare verso le prime 10 posizioni per "' + keyword + '"');
  if (!hasSocial) opps.push('Aprire profili FB+IG: presenza social da costruire da zero');
  else opps.push('Attivare gestione social professionale: da profilo statico a contenuti regolari');
  if (nRating < 50) opps.push('Piano recensioni Google: crescere da ' + nRating + ' verso 50+ per migliorare Maps');

  if (opps.length) {
    h += '<div style="' + secStyle + '">Opportunita Identificate</div>';
    opps.forEach(function(o) {
      h += '<div style="display:flex;gap:10px;background:#fff8e1;border:1px solid #ffe082;border-radius:7px;padding:9px 13px;margin-bottom:7px;font-size:10pt;color:#6d4c00;line-height:1.5">';
      h += '<span style="flex-shrink:0;font-weight:700">+</span><span>' + o + '</span></div>';
    });
  }
...
.analisi-box{border:1.5px solid #e8e8e8;border-left:5px solid #E8001C;border-radius:10px;padding:22px;margin-bottom:28px;overflow:hidden}
...
@media print{body{background:white;print-color-adjust:exact;-webkit-print-color-adjust:exact}.page{box-shadow:none;max-width:100%}.edit-note{display:none}.no-print{display:none}}
...
<div class="body">
  ...
  <div class="edit-note no-print">
    &#9999;&#65039; <strong>Proposta modificabile</strong> &mdash; Clicca sui testi per modificarli &middot; Clicca &#10005; su una riga per rimuoverla
  </div>

  <div class="az-card">
    <div class="az-nome" contenteditable="true">${lead.nome}</div>
    <div class="az-addr">&#128205; ${lead.indirizzo}</div>
    <div class="az-grid">
      <div class="az-row"><span class="az-lbl">Telefono</span><span class="az-val">${lead.telefono !== 'N/D' ? lead.telefono : '&mdash;'}</span></div>
  ...
  <div class="fat-box">
    ...
    <div class="fat-item">
      ...
      <div class="fat-val">&euro; <span>${fatturato.min.toLocaleString('it-IT')}</span> &ndash; <span>${fatturato.max.toLocaleString('it-IT')}</span></div>
      <div class="fat-note">Stima basata su settore, zona e volume clienti</div>
    </div>
    <div class="fat-item">
      <div class="fat-lbl">Budget digitale consigliato</div>
      <div class="fat-val">&euro; <span>${fatturato.budgetMensileMax}</span>/mese</div>
  ...
  <table id="tabella-prodotti">
    ...
    <tbody>
      ...
      <tr class="removable" id="row-${i}" data-anno1="${p.anno1||0}" data-mens="${p.mens||0}">
        ...
        <td class="td-num" id="mens-${i}">${p.mens ? '&euro; ' + p.mens + '/mese' : '&mdash;'}</td>
  ...
  <div class="tot-box">
    <div class="tot-item">
      ...
      <div class="tot-val">&euro; <span id="tot-anno1">${totAnno1.toLocaleString('it-IT')}</span></div>
      <div class="tot-note">IVA esclusa</div>
    </div>
    <div class="tot-div"></div>
    <div class="tot-item">
      <div class="tot-lbl">Canone mensile</div>
      <div class="tot-val">&euro; <span id="tot-mens">${totMens}</span><span style="font-size:13pt;color:rgba(255,255,255,0.4)">/mese</span></div>
  ...
  <div class="foot">
    <div><strong>Pagine Si! SpA</strong> &middot; P.zza San Giovanni Decollato 1, 05100 Terni &middot; paginesispa.it</div>
    <div style="text-align:right">Prezzi al netto di IVA &middot; Proposta valida 30 giorni</div>
  </div>
...
function ricalcolaTotali() {
  ...
  document.querySelectorAll('#tabella-prodotti tbody tr').forEach(tr => {
    anno1 += parseFloat(tr.dataset.anno1 || 0);
    mens  += parseFloat(tr.dataset.mens  || 0);
    num++;
  });
...
function apriPannello() {
  const cats = {
    'Sito Web': ['Si2A-PM','Si2RE-PM','Si2S-PM','Si2VN-PM'],
...
function selezionaSigla(s) {
  document.querySelectorAll('[id^="chip-"]').forEach(el => {
    el.style.background = '#fff'; el.style.borderColor = '#e0e0e0'; el.style.color = '#555';
  });
  _siglaSelezionata = s;
  const chip = document.getElementById('chip-' + s);
  if (chip) { chip.style.background = '#E8001C'; chip.style.borderColor = '#E8001C'; chip.style.color = '#fff'; }
...
function esportaPDF() {
  ...
  document.querySelectorAll('#tabella-prodotti tbody tr').forEach(function(tr) {
    ...
    righeHTML += '<tr style="border-bottom:1px solid #f0f0f0">' +
      '<td style="padding:10px 14px;font-family:monospace;font-size:10px;color:#E8001C;font-weight:700">' + sigla + '</td>' +
      '<td style="padding:10px 14px;font-size:11px;color:#1a1a1a;font-weight:500">' + nome + '</td>' +
      '<td style="padding:10px 14px;font-size:11px;color:#555;text-align:right">' + (a1 ? '\u20ac ' + a1.toLocaleString('it-IT') : '\u2014') + '</td>' +
      '<td style="padding:10px 14px;font-size:11px;color:#555;text-align:right">' + (mn ? '\u20ac ' + mn + '/mese' : '\u2014') + '</td>' +
      '</tr>';
  });
  var mensHTML = totM ? '<div style="text-align:right;margin-left:28px"><div style="font-size:9px;opacity:0.7;margin-bottom:2px">Canone mensile</div><div style="font-size:20px;font-weight:700">\u20ac ' + totM + '/mese</div></div>' : '';
  var html = '<!DOCTYPE html><html lang="it"><head><meta charset="UTF-8"><title>Proposta</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#fff;color:#1a1a1a;padding:36px 44px;max-width:780px;margin:0 auto}@media print{body{padding:20px 28px;print-color-adjust:exact;-webkit-print-color-adjust:exact}}</style></head><body>' +
    '<div style="background:#111;padding:20px 26px;border-radius:10px;margin-bottom:24px;display:flex;align-items:center;justify-content:space-between"><div style="font-size:16px;font-weight:700;color:white">Pagine Si!</div><div style="font-size:10px;color:rgba(255,255,255,0.4);text-align:right">Proposta commerciale<br>' + oggi + '</div></div>' +
    '<div style="border:1px solid #e0e0e0;border-left:4px solid #E8001C;border-radius:8px;padding:16px 20px;margin-bottom:22px"><div style="font-size:16px;font-weight:700;margin-bottom:4px">' + nomeAz + '</div><div style="font-size:11px;color:#888">' + indirizzo + '</div></div>' +
    '<table style="width:100%;border-collapse:collapse;margin-bottom:20px;border:1px solid #e0e0e0"><thead><tr style="background:#f5f5f5"><th style="padding:10px 14px;text-align:left;font-size:9px;color:#888;text-transform:uppercase;letter-spacing:0.08em">Sigla</th><th style="padding:10px 14px;text-align:left;font-size:9px;color:#888;text-transform:uppercase;letter-spacing:0.08em">Servizio proposto</th><th style="padding:10px 14px;text-align:right;font-size:9px;color:#888;text-transform:uppercase;letter-spacing:0.08em">Anno 1</th><th style="padding:10px 14px;text-align:right;font-size:9px;color:#888;text-transform:uppercase;letter-spacing:0.08em">Mensile</th></tr></thead><tbody>' + righeHTML + '</tbody></table>' +
    '<div style="background:#E8001C;color:#fff;border-radius:8px;padding:16px 22px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-size:9px;text-transform:uppercase;letter-spacing:0.08em;opacity:0.7;margin-bottom:4px">Piano di investimento</div><div style="font-size:10px;opacity:0.6">IVA esclusa \u00b7 Valida 30 giorni</div></div><div style="display:flex;align-items:center"><div style="text-align:right"><div style="font-size:9px;opacity:0.7;margin-bottom:2px">Investimento anno 1</div><div style="font-size:20px;font-weight:700">\u20ac ' + tot1.toLocaleString('it-IT') + '</div></div>' + mensHTML + '</div></div>' +
    '<div style="margin-top:14px;font-size:9px;color:#bbb;text-align:center">Pagine Si! SpA \u00b7 paginesispa.it</div></body></html>';
...
<div id="pannello-overlay" onclick="if(event.target===this)chiudiPannello()" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.4);z-index:9999;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:16px;width:680px;max-width:95vw;max-height:84vh;display:flex;flex-direction:column;box-shadow:0 20px 60px rgba(0,0,0,0.2);">
    <div style="padding:18px 22px 14px;border-bottom:1px solid #eee;display:flex;align-items:center;justify-content:space-between">
      <div>
        ...
        <div style="font-size:11px;color:#aaa;margin-top:2px">Listino Pagine Si! 2026 &mdash; seleziona e conferma</div>
      </div>
    ...
    <div style="padding:12px 22px;border-top:1px solid #eee;display:flex;align-items:center;justify-content:space-between;background:#f9f9f9">
      ...
      <button id="btn-aggiungi-ok" onclick="aggiungiServizio()" disabled style="padding:8px 20px;background:#E8001C;color:#fff;border:none;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;opacity:0.4">Aggiungi</button>
    </div>
  </div>
</div>
    ...
    if (sigleExtra && sigleExtra.length > 0) {
      ...
      sigleExtra.forEach(sigla => {
        if (!sigleGiaPresenti.has(sigla) && PRODOTTI[sigla]) {
          prodotti.push({ sigla, ...PRODOTTI[sigla], motivazione: 'Aggiunto manualmente dal consulente', priorita: 10 });
        }
      });
    }

    // Genera proposta HTML originale
    const htmlProposta = generaHTML(lead, prodotti, fatturato, consulente || 'Consulente Pagine Si!');

    // Analisi SEO reale + competitor Maps in parallelo
    const [seoData, mapsCompetitor] = await Promise.all([
      analisiSEOReale(lead),
      cercaCompetitorMaps(lead)
    ]);

    // Genera sezione analisi come HTML puro (string concatenation, zero template literals)
    const analisiHTML = buildAnalisiHTML(seoData, mapsCompetitor, lead);

    // Inietta la sezione analisi DOPO il footer della proposta, prima di </body>
    // In questo modo non interferisce MAI con il template literal di generaHTML
    const analisiSection = '<div style="max-width:900px;margin:24px auto 0;background:white;border-radius:10px;overflow:hidden;box-shadow:0 4px 40px rgba(0,0,0,0.08)">' +
      '<div style="padding:22px">' + analisiHTML + '</div></div>';

    const html = htmlProposta.replace('</body>', analisiSection + '</body>');
...
module.exports = { router, stimaFatturato, costruisciPreventivo, analisiDigitale, PRODOTTI };
