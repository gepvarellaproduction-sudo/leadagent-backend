...
<div class="body">
  ...
  <div class="no-print" style="display:flex;gap:10px;margin-bottom:24px;flex-wrap:wrap;">
    <button onclick="apriPannello()" style="display:inline-flex;align-items:center;gap:6px;padding:9px 20px;background:#fff;border:2px dashed #E8001C;color:#E8001C;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;">
      ＋ Aggiungi servizio
    </button>
...
function esportaPDF() {
  ...
  document.querySelectorAll('#tabella-prodotti tbody tr').forEach(function(tr) {
    ...
    righeHTML += '<tr>' +
      '<td style="padding:9px 12px;border-bottom:1px solid #f0f0f0;font-family:monospace;font-size:10px;color:#E8001C;font-weight:700">' + sigla + '</td>' +
      '<td style="padding:9px 12px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#1a1a1a;font-weight:500">' + nome + '</td>' +
      '<td style="padding:9px 12px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#555;text-align:right">' + (a1 ? '&euro; ' + a1.toLocaleString('it-IT') : '&mdash;') + '</td>' +
      '<td style="padding:9px 12px;border-bottom:1px solid #f0f0f0;font-size:11px;color:#555;text-align:right">' + (mn ? '&euro; ' + mn + '/mese' : '&mdash;') + '</td>' +
      '</tr>';
  });
  var tot1 = 0, totM = 0;
  document.querySelectorAll('#tabella-prodotti tbody tr').forEach(function(tr){
    tot1 += parseFloat(tr.dataset.anno1||0);
    totM += parseFloat(tr.dataset.mens||0);
  });
  var mensHTML = totM ? '<div style="text-align:right;margin-left:32px"><div style="font-size:9px;opacity:0.7">Mensile</div><div style="font-size:17px;font-weight:700">&euro; '+totM+'/mese</div></div>' : '';
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Proposta '+nomeAz+'</title>' +
    '<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;color:#1a1a1a;padding:36px 44px;max-width:780px;margin:0 auto}' +
    '@media print{body{padding:20px 28px;print-color-adjust:exact;-webkit-print-color-adjust:exact}}</style></head><body>' +
    '<div style="background:#111;color:#fff;padding:22px 26px;border-radius:10px;margin-bottom:26px;display:flex;align-items:center;justify-content:space-between">' +
      '<div><div style="font-size:17px;font-weight:700;margin-bottom:3px">'+nomeAz+'</div>' +
      '<div style="font-size:11px;opacity:0.5">Proposta commerciale &middot; '+oggi+'</div></div>' +
      '<div style="font-size:11px;color:#E8001C;font-weight:700;letter-spacing:0.05em">PAGINE S&Igrave;!</div>' +
    '</div>' +
    '<table style="width:100%;border-collapse:collapse;margin-bottom:22px">' +
      '<thead><tr style="background:#f5f5f5">' +
        '<th style="padding:9px 12px;text-align:left;font-size:9px;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Sigla</th>' +
        '<th style="padding:9px 12px;text-align:left;font-size:9px;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Servizio</th>' +
        '<th style="padding:9px 12px;text-align:right;font-size:9px;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Anno 1</th>' +
        '<th style="padding:9px 12px;text-align:right;font-size:9px;color:#aaa;text-transform:uppercase;letter-spacing:0.08em">Mensile</th>' +
      '</tr></thead><tbody>'+righeHTML+'</tbody></table>' +
    '<div style="background:#E8001C;color:#fff;border-radius:8px;padding:15px 20px;display:flex;justify-content:space-between;align-items:center">' +
      '<div style="font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;opacity:0.8">Investimento</div>' +
      '<div style="display:flex;align-items:center">' +
        '<div style="text-align:right"><div style="font-size:9px;opacity:0.7">Anno 1</div><div style="font-size:17px;font-weight:700">&euro; '+tot1.toLocaleString('it-IT')+'</div></div>' +
        mensHTML +
      '</div>' +
    '</div>' +
    '<div style="margin-top:16px;font-size:9px;color:#aaa;text-align:center">Prezzi IVA esclusa &middot; Valida 30 giorni &middot; Pagine S&igrave;! SpA</div>' +
    '</body></html>';
